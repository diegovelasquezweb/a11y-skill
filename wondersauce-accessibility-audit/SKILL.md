...
- Never install dependencies in any directory during audits (including temp/workaround directories such as `/tmp`).
...
7. Before coverage validation, verify `/tmp/wondersauce-a11y-findings.json` exists.
8. If `/tmp/wondersauce-a11y-findings.json` is missing, re-run route-check and findings steps in the same directory, then continue.
...
- Use `/tmp/wondersauce-route-checks.json`, `/tmp/wondersauce-a11y-findings.json`, and `/tmp/wondersauce-a11y-coverage.json` for pipeline JSON files.
...
3. Keep temporary pipeline files in `/tmp`; do not delete them automatically.
