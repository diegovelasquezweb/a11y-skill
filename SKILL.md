...
- Treat the `a11y-audit` CLI as a black box: run with `--help` to discover flags. Do not read engine source — it consumes context budget for no benefit.
...
a11y-audit --base-url <URL>

# Single page
a11y-audit --base-url <URL> --max-routes 1

# All pages or custom count
a11y-audit --base-url <URL> --max-routes <N>  # 999 = all
```
...
a11y-audit --base-url <URL> --max-routes <N> --skip-patterns --affected-only
```
...
5. **Execute** the following commands — do not describe or summarize them, run them:
   ...
   a11y-audit --base-url <URL> --with-reports --output <path>/report.html

   # PDF (run if PDF or Both was selected — same command generates PDF alongside HTML)
   a11y-audit --base-url <URL> --with-reports --output <path>/report.html
   ```
...
If **Yes**, run:
```bash
a11y-audit --base-url <URL> --project-dir <path> [--framework <val>]
```
Then read the updated remediation guide and present any new findings from the "Source Code Pattern Findings" section, grouped by severity. For each pattern group show: severity label, pattern name, number of affected files, and the recommended fix.
...
If **Fix all** or **Let me pick**: apply code fixes following the guidance in the remediation guide. Structural changes (e.g. adding `aria-label`, removing `focus:outline-none`) apply silently. After all fixes are applied, re-run the source scanner to verify:
```bash
a11y-audit --base-url <URL> --project-dir <path> [--framework <val>]
```
...
```bash
a11y-audit --base-url <URL> --with-reports --output <path>/checklist.html
```
